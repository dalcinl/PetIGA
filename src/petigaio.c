...
PetscErrorCode IGALoadGeometry(IGA iga,PetscViewer viewer)
{
  ...
  if (nsd < 1) SETERRQ(((PetscObject)iga)->comm,PETSC_ERR_ARG_WRONGSTATE,"Must call IGASetGeometryDim() first");
  {
    IGA_Grid grid;
...
PetscErrorCode IGALoadProperty(IGA iga,PetscViewer viewer)
{
  ...
  if (npd < 1) SETERRQ(((PetscObject)iga)->comm,PETSC_ERR_ARG_WRONGSTATE,"Must call IGASetPropertyDim() first");
  {
    IGA_Grid grid;
